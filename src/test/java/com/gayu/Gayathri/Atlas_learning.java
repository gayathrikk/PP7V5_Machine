...
public class Atlas_learning {
	...
	public void series_set() throws InterruptedException {
		...
		try {
			WebElement Mouse = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//table[@role='grid']//tr[7]")));
			Mouse.click();
		...
		try {
			WebElement Brain = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//table[@role='grid']//tr[8]//td[2]")));
			Brain.click();
		...
		try {
			WebElement MTB_10 = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//table[@role='grid']//tr[9]//td[3]")));
			MTB_10.click();
		...
		String parentWindow = driver.getWindowHandle();
		try {
			WebElement viewericon = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//nb-icon[@nbtooltip='Viewer']")));
			viewericon.click();
		...
		try {
			WebElement section = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("(//span[text()='370'])[1]")));
			section.click();
	...
	public void atlaseditor() throws InterruptedException {
		...
		driver.switchTo().defaultContent();
		try {
			WebElement atlaseditor = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[@title='Atlas Editor']")));
			atlaseditor.click();
	...
	public void Editmenu() throws InterruptedException {
		...
		driver.switchTo().defaultContent();
		try {
			WebElement Editmenu = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[@title='Edit Menu']")));
			Editmenu.click();
	...
	public void contributor() throws InterruptedException {
		...
		driver.switchTo().defaultContent();
		try {
			WebElement contributor = wait.until(
					ExpectedConditions.elementToBeClickable(By.xpath("(//nb-icon[@pack='nebular-essentials'])[2]")));
			contributor.click();
		...
		try {
			WebElement contributor = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("(//input[@type='radio'])[2]")));
			contributor.click();
...
//		    	 clickElementByXpath("(//i[@role='presentation'])[7]", "node3");
	...
	public void geometry_Dot() throws InterruptedException {
		...
		actions.moveToElement(canvas).click().release().perform();
		Thread.sleep(5000);
